...
function NavDropdown({
    ...
    return (
        <div className="relative" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
            <button
                className={`flex items-center gap-1 text-sm font-medium transition-colors hover:text-brand-accent ${isScrolled ? 'text-brand-text' : 'text-brand-bg-dark'} relative group`}
            >
...
export function Navbar() {
    ...
    useEffect(() => {
        if (!isHome) {
            // eslint-disable-next-line react-hooks/set-state-in-effect -- initial state sync, not cascading
            setIsScrolled(true)
            return
        }
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center">
                    ...
                    <button className="md:hidden z-50 p-2" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
                        {mobileMenuOpen ? (
                            <X
                                className={`w-6 h-6 ${isScrolled || mobileMenuOpen ? 'text-brand-text' : 'text-brand-bg-dark'}`}
                            />
                        ) : (
